...
const About = () => {
  ...
  const anim = useRef();
  useEffect(() => {
    const blocText = ref.current;
    const animation = anim.current;
    window.addEventListener('scroll', () => {
      const { scrollTop, clientHeight } = document.documentElement;
      const topElementToTopViewport = blocText.getBoundingClientRect().top;
      // eslint-disable-next-line no-mixed-operators
      if (scrollTop > (scrollTop + topElementToTopViewport).toFixed() - clientHeight * 0.8) {
        blocText.classList.add('isActif');
        animation.classList.add('isEnter');
      }
    });
  }, []);
    return(
      ...
      <div className="aboutMe__section">
        <div className="aboutMe__section__text" ref={ref}>
        <h2 className="aboutMe__section__text__title">My Background</h2>
        <p className="aboutMe__section__text__description">
        After an intensive training, I acquired the basics of web development.
        </p>
        <p className="aboutMe__section__text__description">I was able to work and learn during all the life cycle of a web application, from the graphic design with the realization of wireframe as well as the conception of database thanks to the MLD, MCD and MPD. </p>
        <p className="aboutMe__section__text__description">Then the integration with html5, css3, JS, and different frameworks like bootstrap, tailwind or bulma. For the backend part I learned to use php, mcd, insomnia for my tests. </p>
        <p className="aboutMe__section__text__description">I also mastered React and redux and the basics of symfony.</p>
        </div>
        <div className="aboutMe__section__animation">
      <svg className="animation" width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" ref={anim}>
      <g id="study">
